...
public class EquipeFacade extends AbstractFacade<Equipe> {
    ...
    public Equipe getEquipe(String id) {
        Equipe equipe = em.find(Equipe.class, Integer.parseInt(id));
        if (equipe == null) {
            Connection con = null;
            try {
                con = DriverManager.getConnection("jdbc:derby://localhost:1527/DB1");
                NamedParameterStatement stmt = new NamedParameterStatement(con, "SELECT e.equi_id, chef_id, nom, avatar, SUM(score) AS scoreTotal FROM equipe e JOIN scoreCTF ON e.equi_id = scoreCTF.equi_id WHERE e.equi_id = :id GROUP BY e.equi_id, chef_id, nom, avatar ORDER BY scoreTotal DESC");
                ...
                equipe = new Equipe();
                equipe.setEquiId(rs.getInt("equi_id"));
                equipe.setNom(rs.getString("nom"));
                equipe.setAvatar(rs.getString("avatar"));
                equipe.setChefId(rs.getInt("chef_id"));
                equipe.setScoreTotal(rs.getInt("scoreTotal"));
                em.persist(equipe);
                con.close();
            } catch (SQLException e) {
                e.printStackTrace();
